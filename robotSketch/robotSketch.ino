...
// Size of buffer (in bytes) for incoming data from the LRF
// (this should be adjusted to be larger than the expected response)
#define BUFSIZE  16
...
String stopString = "#1500,1500$"; // This will cause the servos to stop moving
...
void setup()
{
  ...
  tone(4, 3000, 1000);  // Play tone for 1 second so we know it started
  ...
  */
  ...
  // in case another student wants to fix the functionality
  //Wire.begin();
  ...
  /* if ((rev & 0xF0) != 0x10)
   ...
   tone(4, 3000, 500);  // Play tone for 1 second so we know boot up finished
...
void loop()
{
      ...
      while(exitLoop != 1)
      {
        /* proximitiy sensor not working yet
        prox = readProximity();
        ...
        // if the next byte is a 's' then its the sweep command
        if(test == 's')
        {
          Serial.read();
  	      sweep();
        ...
        // if the next byte is a '#' then its the start of a new motor command string
        if(test == '#' && exitLoop != 1)
        {
          // read until the stop character or we hit 20 bytes
          Serial.readBytesUntil('$', buffer, 20);

          // check for the number of '#' chars in the buffer this validates
          // that the message was recieved successfuly. In semi rare cases
          // the system will get part of two messages resulting in two '#' characters
          // being present in the string
          for(int i = 0; i <= 20; i++)
          {
            if(buffer[i] == '#')
            {
      ...
      // convert to integer for use in servos
      leftMSint = atoi(leftMS);
      rightMSint = atoi(rightMS);

      /* Code to make the robot stop if something is too close to the proxmity sensor
      if(prox > 3000 && leftMSint < 1500 && rightMSint > 1500)
      {
        Serial.print("Prox = ");
        Serial.println(prox);
